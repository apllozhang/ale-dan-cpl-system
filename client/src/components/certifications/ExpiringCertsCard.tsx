...
export function ExpiringCertsCard() {
  ...
  const { data: expiring } = trpc.certifications.expiring.useQuery({ days: 90 });
  ...
  const urgent = expiring.filter((c: { expiryDate: string | null }) => {
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-1">
          {expiring.slice(0, 5).map((cert: { id: number; certNo: string; expiryDate: string | null }) => (
