...
export async function exportQuotationToExcel(quotation: QuotationExportData, items: QuotationItem[]) {
  ...
  if (quotation.validUntil) {
    const validRow = ws.getRow(currentRow);
    validRow.height = 18;
    ws.mergeCells(currentRow, 1, currentRow, 9);
    const validCell = validRow.getCell(1);
    validCell.value = `本报价单有效期至 ${new Date(quotation.validUntil).toLocaleDateString("zh-CN")}`;
    validCell.font = { name: FONT_CN, size: SIZE_FOOTER, italic: true, color: { argb: "CC6600" } };
    validCell.alignment = { horizontal: "center", vertical: "middle" };
    currentRow++;
  }
  ...
  footerCell.value = "本报价仅为参考价格，最终价格以双方签订合同为准。";
  ...
  currentRow++;
  const footerRow3 = ws.getRow(currentRow);
  footerRow3.height = 18;
  ws.mergeCells(currentRow, 1, currentRow, 9);
  const footerCell3 = footerRow3.getCell(1);
  footerCell3.value = "Digital Age Networking · https://www.extremecloudiq.cn/";
  footerCell3.font = { name: FONT_EN, size: 7, color: { argb: "BBBBBB" } };
